...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    #parser.add_argument('groundtruths', type=str, help='Directory containing ground truth files.', default= '/home/shuanghong/Downloads/github/dataset/metrics/gt-dir')
    #parser.add_argument('tests', type=str, help='Directory containing tracker result files', default='/home/shuanghong/Downloads/github/dataset/metrics/ts.dir')
...
def main():
    ...
    groundtruths = '/mnt/disk/shuanghong/dataset/new_dataset_fair_metrics/gt-dir' # ***newdataset***

    tests = '/mnt/disk/shuanghong/on-road-dhu-datasets-code/Towards-Realtime-MOT/results/jde/ts-dir'


    gtfiles = glob.glob(os.path.join(groundtruths, '*/gt/gt.txt')) #for others
    ...
    # for f in gtfiles:
    #     name = Path(f).parts[-3]
    #     file = mm.io.loadtxt(f, fmt=args.fmt)

    # gt = OrderedDict([(Path(f).parts[-3], mm.io.loadtxt(f, fmt=args.fmt, min_confidence=1)) for f in gtfiles])
    gt = OrderedDict([(Path(f).parts[-3], mm.io.loadtxt(f, fmt=args.fmt)) for f in gtfiles])
